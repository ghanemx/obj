...
initialize = True
net = None
dest_dir = './loads' + os.path.sep +'yolo_weights'
classes = None
...
def line_to_speak(labels):
    all_obj = labels
    my_dict = {i: all_obj.count(i) for i in all_obj}
    my_dict
    x = my_dict.keys()
    y = []
    text2 = 'i found'
    if all_obj == []:
        text2= 'no thing detected'
    else:
        for i in x:
            y = str(my_dict[i])
            text2 = str(text2) + ' ' + str(y) + ' ' + str(i)
        type(text2)
    return text2
...
def draw_bbox(img, bbox, labels, confidence, colors= None, write_conf=True):
    ...
    for i, label in enumerate(labels):
        ...
        cv2.putText(img, label, (bbox[i][0],bbox[i][1]+12), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
...
def detect_common_objects(image, confidence=0.5, nms_thresh=0.3, model='yolov4', enable_gpu=False):
    ...
    def draw_bbox(self, img, bbox, labels, confidence, colors, write_conf=False):
